...
if (!function_exists('diza_tbay_center_meta')) {
    function diza_tbay_center_meta($post)
    {
        ...
        "<span class='author'>".esc_html_e('/ By ', 'diza');
...
if (!function_exists('diza_tbay_post_wrapper_share_tags')) {
    ...
    {
        if (!diza_tbay_get_config('enable_code_share', false) && !get_the_tags()) {
            return;
        }
    ...
    function diza_tbay_post_info_author()
    {
		...
		<div class="author-info">
			...
			<div class="content">
				...
				<a href="<?php echo esc_url(get_author_posts_url($authordata->ID, $authordata->user_nicename)); ?>" class="all-post"><?php esc_html_e('See all author post ', 'diza'); ?><i class="tb-icon tb-icon-chevron-right"></i></a>
			</div>
		</div>
		<?php
        }
...
if (!function_exists('diza_tbay_post_share_box')) {
    ...
    {
        if (!diza_tbay_get_config('enable_code_share', false)) {
            return;
        }
...
if (!function_exists('diza_tbay_render_breadcrumbs')) {
    function diza_tbay_render_breadcrumbs()
    {
        ...
        if (diza_tbay_is_home_page() || is_attachment()) {
            return;
        }
    ...
    function diza_tbay_post_nav()
    {
		...
		<nav class="navigation post-navigation">
			...
			<div class="nav-links clearfix">
				<?php
                if (is_attachment()) :
                    previous_post_link('%link', '<div class="col-lg-6"><span class="meta-nav">'.esc_html__('Published In', 'diza').'</span></div>'); else :
...
if (!function_exists('diza_custom_share_code')) {
    ...
    {
        if (!diza_tbay_get_config('enable_code_share', true)) {
            return;
        }
...
if (!function_exists('diza_tbay_back_to_top')) {
    function diza_tbay_back_to_top()
    {
    ...
    add_action('elementor/theme/after_do_footer', 'diza_tbay_back_to_top', 10);
    add_action('diza_after_do_footer', 'diza_tbay_back_to_top', 10);
}
